...
def build_json_export(db: Session, blinded: bool) -> dict:
    ...
    for student in db.scalars(select(Student).order_by(Student.access_code)).all():
        ...
        if not blinded:
            ...
            item["project_title"] = student.project_title
            item["project_goal"] = student.project_goal
...
def build_csv_export(db: Session, blinded: bool) -> str:
    ...
    for session in sessions:
        ...
        base = {
            ...
            "project_title": "" if blinded else (student.project_title or ""),
            "project_goal": "" if blinded else (student.project_goal or ""),
